...
def choose_iwad():
    ...
    print(f"  {DOOM_YELLOW}{BOLD}Choisissez votre IWAD:{RESET}")
    print()
    ...
    if doom1:
        ...
        print(f"  {DOOM_GRAY}[1] DOOM 1  (introuvable){RESET}")

    if doom2:
        print(f"  {DOOM_RED}[2]{RESET} {WHITE}DOOM 2{RESET}  {DOOM_GRAY}({doom2}){RESET}")
        options.append(("doom2", doom2))
    else:
        print(f"  {DOOM_GRAY}[2] DOOM 2  (introuvable){RESET}")

    print(f"  {DOOM_RED}[3]{RESET} {WHITE}Chemin personnalisé{RESET}")
    print()

    while True:
        choice = color_input("Votre choix [1/2/3] →").strip()

        if choice == "1":
            if doom1:
                return doom1
            else:
                error("doom.wad introuvable. Placez-le dans ~/.local/share/doom/ ou ~/doom/")
        elif choice == "2":
            if doom2:
                return doom2
            else:
                error("doom2.wad introuvable. Placez-le dans ~/.local/share/doom/ ou ~/doom/")
        elif choice == "3":
            path = color_input("Chemin vers l'IWAD →").strip().strip("'\"")
            if os.path.isfile(path):
                return path
            else:
                error(f"Fichier introuvable: {path}")
        else:
            warn("Entrez 1, 2 ou 3.")
...
def get_file_interactive():
    print(f"  {DOOM_YELLOW}{BOLD}Glissez-déposez votre fichier .pk3 ou .wad ici,{RESET}")
    print(f"  {DOOM_YELLOW}puis appuyez sur ENTRÉE:{RESET}")
    print()
...
def main():
    ...
    print(f"  {DOOM_GRAY}Pour Linux Terminal{RESET}")
    print()

    # ── Get PWAD path ──
    if len(sys.argv) >= 2:
        pwad_path = sys.argv[1].strip().strip("'\"")
        info(f"Fichier reçu: {DOOM_YELLOW}{pwad_path}{RESET}")
    else:
    ...
    if not os.path.isfile(pwad_path):
        error(f"Fichier introuvable: {pwad_path}")
        sys.exit(1)

    ext = os.path.splitext(pwad_path)[1].lower()
    if ext not in (".pk3", ".wad", ".pk7", ".ipk3"):
        warn(f"Extension inhabituelle: {ext}  (on continue quand même...)")
    ...
    engine = find_engine()
    if not engine:
        error("Aucun moteur Doom trouvé (gzdoom, zdoom, lzdoom, crispy-doom, prboom-plus).")
        print()
        info("Installez GZDoom: https://zdoom.org/downloads")
        sys.exit(1)

    info(f"Moteur détecté: {DOOM_YELLOW}{engine}{RESET}")
    print()
...
if __name__ == "__main__":
    try:
        ...
        print()
        print(f"\n  {DOOM_GRAY}Au revoir, Doomguy.{RESET}\n")
        sys.exit(0)
